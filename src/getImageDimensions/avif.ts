...
export const avif = (bytes: Uint8Array) => {
  if (!isAvif(bytes)) {
    return;
  }
...
function unbox(data: Uint8Array, offset: number) {
...
function getMeta(data: Uint8Array) {
...
// Parses `meta` box
function parseMetaBox(data: Uint8Array, sizes: ImageDimensions[]) {
...
function parseIprpBox(data: Uint8Array, sizes: ImageDimensions[]) {
  let offset = 0;
...
function parseIpcoBox(data: Uint8Array, sizes: ImageDimensions[]) {
  let offset = 0;
...
function getMaxSize(sizes: ImageDimensions[]) {
  let maxSize = sizes[0];
